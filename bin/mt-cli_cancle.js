...
var log = require(path.join(__dirname,'../util/log.js')),
    fisUtil = require(path.join(__dirname,"../util/fis_util.js")),
...
if(program.create){
    inquirer.prompt(createInfo).then(function(answers){
        if(answers.projectName && answers.projectPath && answers.projectType){
            ...
            fisUtil.copy(tempInfo.projectPath,answers.projectPath);
        }
        // if(answers.projectName && answers.projectName !== 'default'){
        //     templeteInfoObject[answers.name]=answers.info;
        //     fs.writeFileSync(templeteInfoJsonPath,JSON.stringify(templeteInfoObject,null,4),'utf8');
        // }else{
        //     log.error('templeteName is null or templeteName is default');
